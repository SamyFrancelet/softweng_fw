...
#include <stdint.h>
  ...
  #include "fsl_pit.h"
#elif McuLib_CONFIG_CPU_VARIANT_RP2040
...
void gprof_init_timer(void) {
  ...
  if (!add_repeating_timer_us(-1000, timer_callback, NULL, &timer)) { /* must match PROF_HZ! */
    McuLog_error("Failed to add timer");
  }
#endif
}
...
int profile_file_write_check(void) {
  ...
  file = fopen ("c:\\tmp\\gprof_test.txt", "w");
  if (file!=NULL) {
    fputs("hello world with file I/O for GNU profiling.\r\n", file);
    (void)fwrite("hello\r\n", sizeof("hello\r\n")-1, 1, file);
    fclose(file);
    return 1; /* ok */
  }
...
void profile_write_data_file(void) {
  writeProfileData();
}